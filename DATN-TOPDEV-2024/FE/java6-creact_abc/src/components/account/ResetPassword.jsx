...
const ResetPassword = () => {
    ...
    useEffect(() => {
        console.log("Current resetStage:", resetStage);
    }, [resetStage]);
    ...
    const handleForgotPassword = async () => {
        ...
        setLoading(true);
        try {
            console.log("Sending forgot password request for email:", resetEmail);
            ...
            if (response.data && response.data.status === 'success') {
                ...
                console.log("Setting resetStage to 'otp'");
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            // Sử dụng axios trực tiếp thay vì apiClient
            const response = await axios.post('http://localhost:8080/api/auth/verify-otp-for-password', 
                null,
                {
                    params: {
                        email: resetEmail,
            ...
            if (response.data && response.data.status === 'success') {
                showSuccessToast(response.data.data || response.data.message);
                console.log("Setting resetStage to 'newPassword'");
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 bg-galaxy p-4">
            <div className="p-8 border-3 w-full max-w-md bg-gray-800 rounded-lg shadow-lg">
                ...
                {resetStage === 'email' && (
                    <div>
                        <h2 className="text-2xl font-semibold text-white text-center mb-6">Quên Mật Khẩu</h2>
                        <p className="text-gray-300 mb-4">Nhập email của bạn để nhận mã xác nhận.</p>
                ...
                {resetStage === 'otp' && (
                    <div>
                        <h2 className="text-2xl font-semibold text-white text-center mb-6">Xác Minh OTP</h2>
                        <p className="text-gray-300 mb-4">Vui lòng nhập mã OTP đã được gửi đến email của bạn.</p>
                ...
                {resetStage === 'newPassword' && (
                    <div>
                        <h2 className="text-2xl font-semibold text-white text-center mb-6">Đặt Lại Mật Khẩu</h2>
                        <p className="text-gray-300 mb-4">Tạo mật khẩu mới cho tài khoản của bạn.</p>
