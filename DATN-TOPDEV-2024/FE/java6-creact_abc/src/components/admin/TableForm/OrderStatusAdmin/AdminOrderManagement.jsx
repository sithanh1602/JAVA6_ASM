...
const AdminOrderManagement = () => {
  ...
  const [activePaymentMethodTab, setActivePaymentMethodTab] =
    useState("Tất cả");
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const pendingOrdersList = ordersData.filter(
  ...
  const handleChangeStatus = async (orderId, newStatus) => {
    try {
      const updatedOrder = await OrderSevice.updateOrderStatus(
        orderId,
        newStatus
      );
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const paymentMethodMatch =
      activePaymentMethodTab === "Tất cả"
        ? true
        : order.paymentStatus ===
          paymentMethodTabs.find((tab) => tab.label === activePaymentMethodTab)
            ?.value;
      ...
      <OrderInfoSection>
        ...
        <OrderInfoGroup>
          ...
          <OrderInfoItem>
            ...
            <OrderInfoValue>
              {data.shoping_Fee ? data.shoping_Fee.toLocaleString() : "0"} VNĐ
            </OrderInfoValue>
          </OrderInfoItem>
          <OrderInfoItem>
            <OrderInfoLabel>Giảm giá:</OrderInfoLabel>
            <OrderInfoValue>
              {data.voucher ? data.voucher.toLocaleString() : "0"} VNĐ
            </OrderInfoValue>
          </OrderInfoItem>
          <OrderInfoItem>
            <OrderInfoLabel>Tổng tiền:</OrderInfoLabel>
            <OrderInfoValue highlight>
              {data.totalPrice.toLocaleString()} VNĐ
            </OrderInfoValue>
          </OrderInfoItem>
        </OrderInfoGroup>
      </OrderInfoSection>
        ...
        {tabs
          ...
          .map((tab) => (
            <ProgressStep key={tab.status}>
              ...
              <ProgressLabel active={data.status >= tab.status}>
                {tab.label}
              </ProgressLabel>
            </ProgressStep>
          ))}
