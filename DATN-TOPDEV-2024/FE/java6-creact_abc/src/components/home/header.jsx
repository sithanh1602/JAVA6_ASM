...
const Header = () => {
    ...
    try {
      ...
      if (token) {
        ...
        if (!decodedToken.userId) {
          console.error("useEffect: No userId found in token");
          setLoading(false);
          return;
        }
        ...
        console.log("useEffect: No token found in localStorage");
        setLoading(false);
      }
    } catch (err) {
      console.error("useEffect: Error decoding token", err);
      setLoading(false);
    }
  }, []);
  ...
  const fetchOrderDetails = async (orderId) => {
    try {
      ...
      const products = await OrderService.getProductsByOrderId(orderId, token);
              ...
              {selectedOrder ? (
                  <>
                    ...
                    <p>
                      ...
                      <span className={selectedOrder.return_order ? "text-red-500" : "text-green-500"}>
                    {selectedOrder.return_order ? "Đã yêu cầu hoàn" : "Không yêu cầu hoàn"}
                  </span>
                    </p>
