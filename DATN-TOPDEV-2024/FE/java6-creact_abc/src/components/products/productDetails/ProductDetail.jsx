...
const ProductDetail = () => {
        ...
        const fetchProductDetails = async () => {
            try {
                ...
                if (productData && productData.length > 0) {
                    ...
                    const fetchedImages = await ProductService.getImagesByProductVariantId(firstVariant.variantId);
                    if (fetchedImages) {
                    ...
                    if (productVariants && productVariants.length > 0) {
                        ...
                        const category = await ProductService.getCategory(productId);
                        setCategory(category);
                        const fetchedImages = await ProductService.getImagesByProductVariantId(firstVariant.idVariants);
                        if (fetchedImages) {
                            setImages(fetchedImages);
                            setMainImage(fetchedImages.length > 0 ? fetchedImages[0].image : null);
                        }
    ...
    const handleVariantSelect = async (variant) => {
        ...
        const fetchedImages = await ProductService.getImagesByProductVariantId(variant.variantId);
        if (fetchedImages) {
            setImages(fetchedImages);
            setMainImage(fetchedImages.length > 0 ? fetchedImages[0].image : null);
            setMainImageIndex(0);
    ...
    const productName = selectedVariant ? selectedVariant.name : "Product not found";
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    ...
                    <div className="p-4 border rounded-lg h-32 overflow-y-auto shadow">
                        ...
                        <p className="text-sm text-gray-600">{selectedVariant.description || "Không có mô tả"}</p>
                    </div>
                </div>
                <div className="flex flex-col justify-between h-[500px]">
                    <div>
                        ...
                        <div className="text-xl font-semibold text-blue-600 mb-4">
                            {selectedVariant.price ? `${selectedVariant.price.toLocaleString()}đ` : "Liên hệ để biết giá"}
                        </div>
                        ...
                        <div className="text-sm text-gray-600 mb-4">
                            <strong>Số lượng:</strong> {selectedVariant.quantity || selectedVariant.stock || 0}
                        </div>
                    </div>
                                ...
                                <div className="flex gap-4 flex-wrap">
                                    {variants.map((variant) => (
                                        <div
                                            ...
                                            ${selectedVariant?.variantId === variant.variantId ? "border-blue-500 bg-blue-100" : "hover:bg-gray-100"}`}
                    ...
                    <div className="space-y-2 mt-4">
                        ...
                        <button className="w-full text-blue-600 border border-blue-600 py-3 px-6 rounded-lg shadow hover:bg-blue-50 transition duration-300">
                            Thêm vào giỏ hàng
                        </button>
                    </div>
                </div>
